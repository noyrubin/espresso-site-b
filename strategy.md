...
### Pulse 62 — NEXT (22:00 IDT Apr 25)
- **SC watch:** Day 22 evening. Watch for any new PAGE 1 entries or clicks on ristretto (pos 8.4) or horchata-latte (pos 3).
- **Content priorities:**
  1. **"pumpkin spice syrup" (estimate ~2-4K diff low)** — our pumpkin-spice-latte has the recipe but a standalone syrup page might rank for the syrup query. DataForSEO check.
  2. **"matcha espresso" 1.6K diff3** — "dirty matcha" angle covered in matcha-frappuccino but a standalone "matcha espresso" guide may fill a gap. SERP check.
  3. **New cluster scan** — "rose latte" 1K diff12 (SERP check), "honey lavender latte" 1.3K diff2 (beatable SERP likely), "brown butter latte" 390/mo (too low), "pistachio cream cold brew" 1K diff3 (may write)
  4. **Tea latte cluster continuation** — After chai + london fog + strawberry matcha, what's next? "matcha latte variations" or "thai iced tea latte" if volume warrants.
  5. **Keyword ideas batch** — Run keyword-ideas for "espresso drinks" or "coffee recipes" to surface any remaining 5K+ ULTRA-LOW gems we haven't found yet.
- **SEO skill:** Ch7.1 (cluster measurement: tea latte cluster = london-fog + strawberry-matcha + chai-latte = 3 pages; golden/wellness cluster = 1 page; chai cluster complete with dirty + iced + classic), Ch4.1 (continue fan-out), Ch5.2 (verify no new orphans from 158 pages)
